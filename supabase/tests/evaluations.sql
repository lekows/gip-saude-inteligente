...
set local role authenticated;
select pg_temp.login(14);
select public.save_evaluation_response(pg_temp.f(110), '{"course_review":"Comentario sintetico"}'::jsonb, false, 0);
select pg_temp.assert_true((select status='draft' and revision=1 and answers->>'course_review'='Comentario sintetico' from public.evaluation_responses where campaign_id=pg_temp.f(110)), 'course comment draft survives without a rating');
...
select public.save_evaluation_response(pg_temp.f(110), '{"course_rating":4,"course_review":"Comentario sintetico"}'::jsonb, true, 1);
select public.save_evaluation_response(pg_temp.f(110), '{"course_rating":4,"course_review":"Comentario sintetico"}'::jsonb, true, 1);
select pg_temp.assert_true((select count(*)=1 from public.evaluation_response_versions where answers='{"course_rating":4,"course_review":"Comentario sintetico"}'::jsonb), 'course submission preserves exact answers and retry adds no duplicate');
select pg_temp.login(15);
...
select public.save_evaluation_response(pg_temp.f(110), '{"course_rating":5}'::jsonb, true, 0);
select pg_temp.assert_true((select status='submitted' and answers='{"course_rating":5}'::jsonb from public.evaluation_responses where campaign_id=pg_temp.f(110)), 'course can be submitted without optional comment');
select pg_temp.login(3);
