import { COURSE_FIELDS, isCourseEvaluation, SELF_RATINGS, SELF_TEXTS } from "@/lib/evaluations/rules";
...
export function AnswerSummary({ answers }: { answers: EvaluationAnswers }) {
  return <dl className="grid gap-4 sm:grid-cols-2">
    {Object.entries(isCourseEvaluation(answers) ? COURSE_FIELDS : { ...SELF_RATINGS, ...SELF_TEXTS }).map(([key, label]) => <div className="min-w-0" key={key}>
