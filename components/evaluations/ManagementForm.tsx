...
export function CampaignForm({ cycles, classes, action }: {
  ...
  return <ManagementForm action={action} label="Criar avaliação" resetOnSuccess>
    <div className="grid gap-4 sm:grid-cols-2">
      <label className="grid gap-2 text-sm font-semibold" htmlFor={`${prefix}-title`}>Título<input id={`${prefix}-title`} className="rounded-md border border-stone-300 bg-white px-3 py-2 font-normal" name="title" required minLength={5} maxLength={160} placeholder="Ex.: Avaliação do curso" /></label>
      <label className="grid gap-2 text-sm font-semibold" htmlFor={`${prefix}-kind`}>Tipo<select id={`${prefix}-kind`} className="rounded-md border border-stone-300 bg-white px-3 py-2 font-normal" name="kind" value={kind} onChange={(event) => setKind(event.target.value)}><option value="self">Avaliação do curso (identificada)</option><option value="program">Avaliação anônima do GIP</option></select></label>
