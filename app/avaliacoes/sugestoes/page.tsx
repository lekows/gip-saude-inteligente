...
import { CourseSuggestionForm } from "@/components/evaluations/CourseSuggestionForm";
import { evaluationPageContext } from "@/lib/evaluations/server";
import { submitSuggestion } from "@/app/avaliacoes/actions";

export default async function SuggestionsPage({ searchParams }: { searchParams: Promise<{ etapa?: string }> }) {
  const followUp = (await searchParams).etapa === "melhoria";
  ...
  return <EvaluationLayout title="Sugestões de melhoria" description="Conte o que pode melhorar e, se quiser, proponha uma solução. A coordenação publica os encaminhamentos no mural de melhorias.">
    {data?.length ? followUp ? <CourseSuggestionForm onSubmit={submitSuggestion} /> : <SuggestionForm onSubmit={submitSuggestion} /> : <p className="rounded-lg border border-amber-200 bg-amber-50 p-6 text-sm leading-6">O envio é destinado aos participantes com vínculo ativo em uma turma. Peça à coordenação para conferir seu vínculo.</p>}
  </EvaluationLayout>;
}
