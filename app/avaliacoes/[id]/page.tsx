...
export default async function EvaluationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <EvaluationLayout title={campaign.title} description={`${campaign.kind === "self" ? "Conte o que você achou do curso" : "Avaliação anônima das atividades do GIP"}. Período: ${evaluationDate(campaign.opens_at)} a ${evaluationDate(campaign.closes_at)} (horário de Brasília).`}>
    {campaign.kind === "program" ? open ? <ProgramEvaluationForm campaignId={id} onSubmit={submitProgramEvaluation} /> : <p className="rounded-lg border border-stone-200 bg-white p-6">Esta avaliação não está recebendo respostas. Os envios anônimos não ficam no histórico pessoal.</p> : <>
      ...
      {response?.status === "submitted" && <p className="text-sm leading-6"><Link href="/avaliacoes/sugestoes?etapa=melhoria" className="font-semibold text-folha hover:underline">Continuar para sugestão ou crítica anônima</Link>. Esse texto será guardado separadamente e não aparecerá neste histórico.</p>}
