...
import { canManageEvaluations, canReviewEvaluations } from "@/lib/evaluations/rules";
...
export async function evaluationContext(mode: "member" | "reviewer" | "manager" = "member") {
  ...
  if (profileError || !profile || profile.account_status !== "aprovado" || !profile.active) {
    throw new Error("É necessário ter uma conta aprovada e ativa.");
  }
...
export async function getMyEvaluations() {
  ...
  if (campaigns.error || responses.error || publications.error || membership.error) throw new Error("O módulo de avaliações está temporariamente indisponível.");
  ...
  if (feedback.error) throw new Error("Não foi possível carregar as devolutivas.");
